...
def deal_with_garbled(ids, decoding):
    for id in ids:
        ...
        # 更新数据库中的记录
        db.cursor.execute(
            "UPDATE page SET title = %s, content = %s WHERE id = %s",
            (title, content, id))
...
def create_duplicate_title_table():
    # 检查表是否存在
    db.cursor.execute("SHOW TABLES LIKE 'same_title'")
...
def modify_same_title_content(id_start, id_end):
    ...
    db.cursor.execute("SELECT id, title FROM page WHERE id BETWEEN %s AND %s",
                      (id_start, id_end))
    records = db.cursor.fetchall()
    # 按title分组记录
    ...
    for title, ids in title_dict.items():
        if len(ids) > 1:
            ...
            for i, id in enumerate(ids):
                ...
                db.cursor.execute("UPDATE page SET title = %s WHERE id = %s",
                                  (new_title, id))
    db.cnx.commit()
...
def delete_links():
    ...
    for id in ids:
        ...
        new_links = '\n'.join(links_to_keep)
        db.cursor.execute("UPDATE page SET links=%s WHERE id=%s",
                          (new_links, id))
...
def delete_duplicate_urls():
    # 找出重复的url和它们的id
    db.cursor.execute(
        "SELECT url, GROUP_CONCAT(id) FROM page GROUP BY url HAVING COUNT(url) > 1"
    )
    ...
    for url, ids in duplicate_urls:
        ...
        # 保留每组重复记录中的第一个，删除其余的
        id_list.pop(0)
        db.cursor.execute("DELETE FROM page WHERE id IN (%s)" %
