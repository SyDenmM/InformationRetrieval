...
stop_words = [
    "的", "了", "在", "是", "我", "有", "和", "就", "不", "人", "他", "这", "中", "大", "以",
    "到", "说", "等", "能", "也", "上", "或", "之", "但", "个", "都", "而", "啊", "把", "那",
    "你", "一", "为", "所", "年", "没", "着", "要", "与"
]
...
def deal_query(query):
    ...
    try:
        # 尝试执行查询
        query, global_results = global_query.search(query)
        total_page = (len(global_results) + 9) // 10  # 总页数，避免空列表导致错误
        page_number = 1
        results = global_results[:10]
        return query, results
    except Exception as e:
        # 记录具体错误信息
        print(f"Error in deal_query with query '{query}': {e}")
        return None, []
...
def result():
    ...
    return render_template('result.html',
                           pop_query=pop_query[:8],
                           query=query,
                           number=len(global_results),
                           cost_time=cost_time,
...
"""
                           ...
                           recommend_query1=recommend_query1,
                           recommend_query2=recommend_query2,
                           highlight=highlight)
                           """
