...
class Query:
    ...
    def get_pop_query(self, n):
        ...
        # 查询语句
        query = (
            "SELECT query_content FROM user_queries ORDER BY count DESC LIMIT %s"
        )
        # 执行查询
    ...
    def save_query_log(self, type, query):
        ...
        if type in ['<basic_search>', '<phase_search>', '<fuzz_search>']:
            # 检查query_content是否存在
            check_query = (
                "SELECT count(*) FROM user_queries WHERE query_content = %s")
            self.cursor.execute(check_query, (query, ))
            result = self.cursor.fetchone()
            if result[0] > 0:  # query_content存在，更新行
                update_query = (
                    "UPDATE user_queries SET count = count + 1, query_time = %s WHERE query_content = %s"
                )
                query_data = (query_time, query)
            else:  # query_content不存在，插入新行
                update_query = (
                    "INSERT INTO user_queries (query_content, query_time, count) VALUES (%s, %s, 1)"
                )
                    ...
                    "function_score": {
                        ...
                        "functions": [{
                            "script_score": {
                                "script": {
                                    "source":
                                    "doc['pageRank'].value == 0 ? 1 : Math.log1p(doc['pageRank'].value * params.factor)",
                                    "params": {
                                        "factor": 100000
                                    }
                                }
    ...
    def bool_search(self,
        ...
        position参数用于指定搜索的位置
        '''
        save_query = "[must]:" + str(must) + "\t[mustnot]:" + str(
            mustnot) + "\t[should]:" + str(should) + "\t[position]:" + str(
                position)
    ...
    def wildcard_search(self, query):
        ...
        # 执行搜索
        response = self.es.search(index="web",
                                  body={
                                      "query": {
                                          "bool": {
                                              "should": [{
    ...
    def search(self, query):
        original_query = query  # 复制原始query，以便在整个方法中保持原始值不变

        if query[0] == '"':
            # 短语查询
            print(f"Executing phrase search with query: {query}")
            results = self.phase_search(query[1:-1])
            query = query[1:-1]
        elif query[0] == '#':
            # 高级查询
            if query[1:6] == 'regex':
                print(f"Executing regex search with query: {query}")
                results = self.regexp_search(query[7:])
                query = query[7:]
            elif query[1:9] == 'wildcard':
                print(f"Executing wildcard search with query: {query}")
                results = self.wildcard_search(query[10:])
                query = query[10:]
            elif query[1:5] == 'site':
                print(f"Executing site search with site: {query}")
                site = query.split(' ')[0][6:]
                query = query[len(site) + 7:]
                results = self.site_search(site, query)
            else:
                if query[1:6] == 'title':
                    field = 'title'
                    ...
                    print('Error: Unrecognized field in advanced query.')
                    raise Exception("Field parsing error")
                must, must_not, should = self.get_bool_querys(query)
                results = self.bool_search(must, must_not, should, field)
        elif query[0] == '+' or query[0] == '-' or query[0] == '|':
            print(f"Executing full boolean search with query: {query}")
            must, must_not, should = self.get_bool_querys(query)
            results = self.bool_search(must, must_not, should)
        else:
            # 普通查询
            print(f"Executing basic search with query: {query}")
            results = self.basic_search(query)

        if len(results) < 50:
            print("Insufficient results, executing fuzzy search.")
            fuzz_results = self.fuzz_search(
                original_query)  # 使用original_query进行模糊搜索
            for result in fuzz_results:
                if result not in results:
                    results.append(result)

        return original_query, results  # 返回原始的query值
